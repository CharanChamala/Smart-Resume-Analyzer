...
def rank_resumes(job_description, resumes):
    ...
    for resume in resumes:
        ...
        # Calculate TF-IDF representation for the resume
        resume_tfidf = vectorizer.transform([resume])

        # Calculate cosine similarity between job description and resume
        similarity_score = cosine_similarity(job_tfidf, resume_tfidf)[0][0]
        
        rankings.append(similarity_score)
    
    # Sort resumes based on their ranking (higher similarity score is better)
    ranked_resumes = sorted(zip(resumes, rankings), key=lambda x: x[1], reverse=True)
    
    return ranked_resumes

def ranking(rawdata):
    job_description = st.text_area("Enter the job description")
    resumes=np.array(rawdata)
    l=[]
    for i in resumes:
        for j in i:
            l.append(j)
    ...
    if st.button("Rank Resumes"):
        st.write("hello")
        # ranked_resumes = rank_resumes(job_description, l)
        # for resume, rank in ranked_resumes:
...
def run():
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'admin' and ad_password == 'charan':
                st.success("Welcome Chamala Giri Charan")
                # Display Data
                cursor.execute('''SELECT*FROM user_data''')
                c1.execute('''select Predicted_Field from user_data''')
                data1=c1.fetchall()
                df1=pd.DataFrame(data1,columns=['domain'])
                st.dataframe(df1)
